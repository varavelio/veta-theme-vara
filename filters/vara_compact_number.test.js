...
import compactNumber from "./vara_compact_number.js";

test("vara_compact_number returns values below one thousand without a suffix", () => {
...
test("vara_compact_number formats thousands with a k suffix", () => {
...
test("vara_compact_number formats millions with an M suffix", () => {
...
test("vara_compact_number preserves the sign when formatting negative values", () => {
  assert.equal(compactNumber({}, -42.5), "-42.5");
  assert.equal(compactNumber({}, -999.9), "-999.9");
  assert.equal(compactNumber({}, -1000), "-1k");
  assert.equal(compactNumber({}, -1200), "-1.2k");
...
test("vara_compact_number converts numeric input values", () => {
  assert.equal(compactNumber({}, "1200"), "1.2k");
  assert.equal(compactNumber({}, "1000000"), "1M");
  assert.equal(compactNumber({}, "-1200"), "-1.2k");
...
test("vara_compact_number returns zero for non-finite input", () => {
