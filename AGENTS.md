...
- `docs_search` enables local client-side documentation search and defaults to `true`. Each docs root must provide a sibling index page using `vara-docs-search-index`; its filename is configured by `docs_search_index_filename` and defaults to `vara-docs-search-index.json`. The UI reports a configuration error if it is missing. Every `vara-docs` page is included unless it sets `disable_search: true`. Search reuses the rendered heading-ID algorithm so section result hashes match the table of contents.
